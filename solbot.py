...
@app.route("/")
def index():
    return jsonify({"status": "running", "message": "Solana Telegram trading bot active"})
...
async def monitor_trade(app_obj, ca, symbol, entry_price, amount_in_lamports):
    ...
    while True:
        try:
            ...
            pairs = r.get("pairs", [])
            if not pairs:
                await app_obj.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"No price for {symbol}, retrying...")
            ...
            await app_obj.bot.send_message(chat_id=TELEGRAM_CHAT_ID, text=f"Error monitoring {symbol}: {e}")
